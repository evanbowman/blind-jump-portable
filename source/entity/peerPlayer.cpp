...
{
    if (distance(position_, info.position_) > 6) {
        position_ = info.position_;
        interp_offset_ = info.position_ - position_ ;
    } else {
        position_ = info.position_;
    }
...
void PeerPlayer::update(Platform& pfrm, Game& game, Microseconds dt)
{
    ...
    Vec2<Float> new_pos{position_.x - ((speed_.x) * dt * MOVEMENT_RATE_CONSTANT),
                        position_.y - ((speed_.y) * dt * MOVEMENT_RATE_CONSTANT)};

    interp_offset_ = interpolate(Vec2<Float>{0.f, 0.f}, interp_offset_, dt * 0.000000006f);
